<template>
  <Dialog @clickOutside="close" class="indicator-library-dialog">
    ...
    <div class="form-group">
      ...
      <table v-if="filteredIndicators.length" class="table table--inset">
        <thead>
          <tr>
            <th
            ...
            <th class="table-sortable" @click="toggleSort('uses')">
              <span>Use</span>
              <i
                v-if="sortProperty === 'uses'"
                class="table-sortable__direction icon-up"
                :class="[
                  sortDirection < 0 && 'table-sortable__direction--desc'
                ]"
    ...
    <template v-slot:footer>
      <button class="btn -blue -large -cases -file">
        <input
          ...
          ref="importButton"
        />
        Browse <i class="icon-upload ml8"></i>
      </button>
      <button
        class="mlauto btn -green -large -cases"
        @click="promptCreateIndicator"
      >
        Create <i class="icon-plus ml8"></i>
      </button>
    </template>
  </Dialog>
</template>
...
<script>
import { ago, downloadAnything, slugify, uniqueName } from '@/utils/helpers'
...
export default {
  ...
  computed: {
    ...
    sortFunction() {
      if (this.sortProperty === 'uses') {
        if (this.sortDirection > 0) {
          return (a, b) => a.uses - b.uses
        }
        return (a, b) => b.uses - a.uses
      } else if (this.sortProperty === 'description') {
        if (this.sortDirection > 0) {
          return (a, b) =>
